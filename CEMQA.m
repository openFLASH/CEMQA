% Contributors:
% Lucian Hotoiu (open.reggui@gmail.com)
% Rudi Labarbe 
...
% Define image names
scan_CEM_imageName = 'scan_CEM';
air_CT_imageName = 'air_CT';

% Display one slice of plan CEM for information
[pre_ref_cem_data, pre_ref_cem_info, ~] = Get_reggui_data(pre_handles, air_CT_imageName);
...
minTot = min(pre_scan_CEM_data,[],'all');
maxTot = max(pre_scan_CEM_data,[],'all');
%for i = 1:size(pre_scan_CEM_data,3)
i=250; %150
figure(11)
imagf = squeeze(pre_scan_CEM_data(:,:,i));
image((imagf - minTot) ./ (maxTot  - minTot) .* 255)
...
pre_ref_cem_info.Spacing = pre_ref_cem_info.Spacing([1,3,2]); % invert 3 and 2 dimension according to reggui handles, only for the purpose of this local resampling
grid.origin = [1; 1; 1];
grid.size = floor((size(pre_scan_CEM_data)' ./ (pre_ref_cem_info.Spacing ./ pre_scan_CEM_info.Spacing)));
grid.spacing = pre_ref_cem_info.Spacing;
...
handles = loadAlldatasets(scanCEM_path , CT_air_path , RSplanFileName , outputPath , Plan , permOrder, flipAxis, source);

% Import scanned CEM
[scan_CEM_data, scan_CEM_info, ~] = Get_reggui_data(handles, scan_CEM_imageName);

% Save scan to disk
handles = save2Disk(handles, scan_CEM_data, size(scan_CEM_data), scan_CEM_info, 'scan_CEM_image', outputPath);           

% Import ref CEM
ref_CEM_imageName = 'air_CT';
[ref_cem_data, ~, ~] = Get_reggui_data(handles, ref_CEM_imageName);

figure(9977)
histogram(ref_cem_data,255);
xlabel('HU');
ylabel('# of voxels');
...
% Rigid registration normalised scan and reference cem
...
handles = save2Disk(handles, scan_CEM_mask_data, size(scan_CEM_mask_data), scan_CEM_mask_info, scan_CEM_mask, fullfile(handles.dataPath));
[ref_cem_mask_data, ref_CEM_mask_info, ~] = Get_reggui_data(handles, ref_cem_mask);
...
mask_difference_3D = scan_CEM_mask_data - ref_cem_mask_data;


% Scan geometrical dimensions
fprintf('Computing geometrical differences (scan - ref) \n');
scan_distance_map2D = squeeze(sum(scan_CEM_mask_data, 3)).* pxlSizeZ;
...
difference3D_mask_name = 'scan-ref_cem_mask_diff';
difference3D_CT_info = handles.images.info{2}; % copy CT info from CEM if existent
...
if(geomGammaIndexAnalysis)
    ...
    [gamma, myMask, myIm1_resampled] = gamma_2D(scan_distance_map2D, scan_CEM_mask_info, ref_distance_map2D, ref_CEM_mask_info, [], options);
...
fprintf('STD HU resampled/eroded scanned CEM is equal to %d. \n', std_HU_scan);
fprintf('Min HU resampled/eroded scanned CEM is equal to %d. \n', min_HU_scan);
fprintf('Max HU resampled/eroded scanned CEM is equal to %d. \n', max_HU_scan);

fprintf(fileID,'\nMean HU resampled/eroded scanned CEM is equal to %d. \n', mean_HU_scan);
fprintf(fileID,'STD HU resampled/eroded scanned CEM is equal to %d. \n', std_HU_scan);
fprintf(fileID,'Min HU resampled/eroded scanned CEM is equal to %d. \n', min_HU_scan);
fprintf(fileID,'Max HU resampled/eroded scanned CEM is equal to %d. \n', max_HU_scan);
fclose(fileID);
...
outlier_scan_CEM_imageName = 'outliers_scan_CEM';
outlier_scan_CEM_data = scan_CEM_data;
outlier_scan_CEM_data(outlier_scan_CEM_data < -900) = NaN; % exclude air
outlier_scan_CEM_data(outlier_scan_CEM_data > (mean_HU_scan-3*std_HU_scan) & scan_CEM_data < (mean_HU_scan+3*std_HU_scan)) = NaN; % exclude CEM densities
handles = Set_reggui_data(handles, outlier_scan_CEM_imageName, outlier_scan_CEM_data, scan_CEM_info,'images',1);
...
% plot volume histogram
figure(1008);
histogram(reshape(outlier_scan_CEM_data,size(outlier_scan_CEM_data,1)*size(outlier_scan_CEM_data,2)*size(outlier_scan_CEM_data,3),1,1),255);
xlabel('HU');
ylabel('# of voxels');
...
if (weplGammaIndexAnalysis)
    ...
    WEPL_Z_CEM_scan = hu_to_we(scan_CEM_data, 'reggui_material_pmma_phantom.txt');
    WEPL_scan_2Dmap = sum(WEPL_Z_CEM_scan, 3, 'omitnan').* pxlSizeZ;
    
    handles = save2Disk(handles, WEPL_Z_CEM_scan, size(WEPL_Z_CEM_scan), difference3D_CT_info, 'scan_CEM_WEPL_3D', fullfile(handles.dataPath));
    
    
    WEPL_Z_CEM_ref = hu_to_we(def_ref_cem_data, 'reggui_material_pmma_phantom.txt');
    WEPL_ref_2Dmap = sum(WEPL_Z_CEM_ref, 3, 'omitnan').* pxlSizeZ;
    
    handles = save2Disk(handles, WEPL_Z_CEM_ref, size(WEPL_Z_CEM_ref), difference3D_CT_info, 'ref_CEM_WEPL_3D', fullfile(handles.dataPath));
    
    WEPL_diff_3Dmap = (WEPL_Z_CEM_scan - WEPL_Z_CEM_ref);
...
for i = 1:length(figHandles)
    fig = figHandles(i);
    filename = fullfile(outputPath,sprintf('figure_%d.png', fig.Number));
...
% #########################################################################
function pre_handles = loadAlldatasets(scanCEM_path, CT_air_path, RSplanFileName, outputPath, Plan, permOrder, flipAxis, source)
    ...
    switch source
        case 'CT'
            ref_CEM_imageName = 'ref_CEM';
            [ref_CEM_directory, ref_CEM_fileName, EXT] = fileparts(refCEM_path);
            pre_handles = Import_data(ref_CEM_directory, [ref_CEM_fileName EXT], 1, ref_CEM_imageName, pre_handles);
            [pre_ref_cem_data, pre_ref_cem_info, ~] = Get_reggui_data(pre_handles, ref_CEM_imageName);
    ...
    % Import scanned CEM 
    scan_CEM_imageName = 'scan_CEM';
    [scan_CEM_directory, scan_CEM_fileName, EXT] = fileparts(scanCEM_path);
    pre_handles = Import_image(scan_CEM_directory, [scan_CEM_fileName EXT],1,scan_CEM_imageName,pre_handles); %Load CEM image into handles.data
